            ...
            if labels is not None:  
                for i,(x, y) in enumerate(pcs[[d1,d2]].T):
                    if x >= xmin and x <= xmax and y >= ymin and y <= ymax :
                        plt.text(x, y, labels[i], fontsize='14', ha='left', va='center',rotation=label_rotation, color="black", alpha=1)  #2.5 factor to superpose with scatter
...
def display_scree_plot(pca):
    ...
    legend_elements = [
                        Line2D([0], [0], marker='o', color='red', label='Cumulative explained variance',markerfacecolor='red', markersize=8),
